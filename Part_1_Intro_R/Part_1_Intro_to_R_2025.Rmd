...
#3: Identify title, gender and race of the highest paid position at NYT  
NYT %>% 
  select(Salary, TITLE, Gender, Race, COMPANY) %>% 
 group_by(Salary = 350000)
```
...
#Which one is higher?
Business <- subset(MediaSalary, grepl("?Business", COMPANY))
Business %>% 
  select(Salary, TITLE, Gender, Race, COMPANY) %>% 
 group_by(Salary)
...
Bloom <- subset(MediaSalary, grepl("?Bloomberg", COMPANY))
Bloom %>% 
  select(Salary, TITLE, Gender, Race, COMPANY) %>% 
 group_by(Salary)
...
#Sum of Business salaries is 765,800. Sum of Bloomberg salaries is 1,155,540. Sum of Bloomberg salaries are higher. 
...
BigBoys %>% 
  select(Salary, TITLE, Gender, Race, COMPANY, YEARS_EXPERIENCE) %>% 
 group_by(Salary)
```
...
Reporters %>% 
  select(Salary, TITLE, Gender, Race, COMPANY) %>% 
 group_by(Salary = 230504)
 #Lots of folks who are different types of reporters and working at different places! 
```
...
#The Median is 60,240
```
3: Find highest paid editor. Resent them. 
```{r}
Editors %>% 
  select(Salary, TITLE, Gender, Race, COMPANY) %>% 
 group_by(Salary = 245000)
 #There are a few highest paid editors. We resent them!
```
```
